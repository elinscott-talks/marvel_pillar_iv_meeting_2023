...
\begin{frame}{Koopmans functionals: theory}
   \hbox{
      \begin{minipage}{0.5\textwidth}
         ...
         \onslide<3->{
         \vspace{1ex}
         For DFT, this condition is \emph{not} satisfied
         }
         %
      \end{minipage}
...
\begin{frame}{Koopmans functionals: results for toy systems}
   For Hooke's atom (two electrons in a harmonic confining potential)
...
\begin{frame}{Koopmans functionals: caveats}
   ...
   \begin{itemize}[<+->]
      ...
      \item can potentially break the crystal point group symmetry
   \end{itemize}
\end{frame}
...
\begin{frame}{Recent advances: easier Wannierization}
...
\begin{frame}{Recent advances: screening parameters via DFPT}
...
\begin{frame}{Recent advances: screening parameters via ML}
   \begin{center}
      \begin{tikzpicture}
